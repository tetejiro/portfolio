...
<body>
  ...
  //パスワードかぶり
  $rec = $DbQuery->dbQuery('SELECT name FROM member where pass =\''.hash('sha512', $pass).'\'');
  if (empty($rec) == false) {
  ...
  $mailQuery = $DbQuery->dbQuery('SELECT mail FROM member WHERE mail = \''.$mail.'\'');
